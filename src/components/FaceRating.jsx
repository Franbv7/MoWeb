...
const StyledRating = styled(Rating)(({ theme, darkMode }) => ({
  "& .MuiRating-iconEmpty .MuiSvgIcon-root": {
    color: darkMode ? "gray" : theme.palette.action.disabled,
  },
}));
...
export default function MovieRating({ popularity }) {
  const { darkMode } = useStateContext(); // Obtener darkMode desde el contexto
  ...
  return (
    <StyledRating
      ...
      getLabelText={(value) => customIcons[value].label}
      highlightSelectedOnly
      darkMode={darkMode} // Pasar darkMode como prop a StyledRating
    />
  );
}
